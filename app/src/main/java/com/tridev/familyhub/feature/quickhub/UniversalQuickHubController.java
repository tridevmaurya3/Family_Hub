...
public final class UniversalQuickHubController {
    ...
    private static void stopLegacy(Context c){c.startService(new Intent(c,GroceryOverlayService.class).setAction(GroceryOverlayService.ACTION_STOP));c.startService(new Intent(c,FamilyTaskOverlayService.class).setAction(FamilyTaskOverlayService.ACTION_STOP));}
}
