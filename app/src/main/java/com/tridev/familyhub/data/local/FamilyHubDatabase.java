...
public abstract class FamilyHubDatabase extends RoomDatabase {
    ...
    /** Adds human-readable task audit names without changing existing task identity. */
    ...
    /** Adds stable family-cloud identity and member attribution to Finance entries. */
    private static final Migration MIGRATION_23_24 = new Migration(23, 24) {
        @Override public void migrate(SupportSQLiteDatabase database) {
            database.execSQL("ALTER TABLE `finance_entries` ADD COLUMN `cloudId` TEXT NOT NULL DEFAULT ''");
            database.execSQL("ALTER TABLE `finance_entries` ADD COLUMN `familyId` TEXT NOT NULL DEFAULT ''");
            database.execSQL("ALTER TABLE `finance_entries` ADD COLUMN `updatedByUid` TEXT NOT NULL DEFAULT ''");
            database.execSQL("ALTER TABLE `finance_entries` ADD COLUMN `updatedByName` TEXT NOT NULL DEFAULT ''");
    ...
    /** Adds optional smart-form metadata without rewriting existing reminders. */
    ...
    private static final Migration MIGRATION_33_34 = new Migration(33, 34) {
        @Override public void migrate(SupportSQLiteDatabase database) {
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `seenAt` INTEGER NOT NULL DEFAULT 0");
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `acceptedAt` INTEGER NOT NULL DEFAULT 0");
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `startedAt` INTEGER NOT NULL DEFAULT 0");
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `completedAt` INTEGER NOT NULL DEFAULT 0");
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `completedByName` TEXT NOT NULL DEFAULT ''");
        }
    };
    private static final Migration MIGRATION_34_35 = new Migration(34, 35) {
        @Override public void migrate(SupportSQLiteDatabase database) {
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `relatedModule` TEXT NOT NULL DEFAULT ''");
            database.execSQL("ALTER TABLE `reminders` ADD COLUMN `relatedItemTitle` TEXT NOT NULL DEFAULT ''");
        }
    };
